# Versioning — ask-ruby-harness-mcp
...
ask-ruby-harness-mcp is pre-1.0 (`0.y.z`). Until 1.0.0:

- **PATCH** (`0.1.8` → `0.1.9`) — bug fixes and other
  backwards-compatible corrections. No new features, no public API changes.
- **MINOR** (`0.1.8` → `0.2.0`) — new features **and**
...
Patch numbers increase by exactly one per release. From `0.1.8` the next
patch is `0.1.9` — never `0.1.11`, never a skip, never a jump. Patch
...
Every `ask-*` gem — ask-ruby-harness-mcp included — and `yamine` is released
...
gemchain guard ask-ruby-harness-mcp          # blast radius: who depends on us
gemchain update ask-ruby-harness-mcp 0.1.9 --dry-run
gemchain update ask-ruby-harness-mcp 0.1.9 --test-only
gemchain update ask-ruby-harness-mcp 0.1.9
```
...
2. **Version** — bump `lib/ask/ruby/mcp/version.rb` by exactly one step
   (gemchain does this).
...
5. **Build** — `gem build ask-ruby-harness-mcp.gemspec` succeeds and the
...
- `lib/ask/ruby/mcp/version.rb` at `HEAD`
- the `vX.Y.Z` git tag
...
- Fix a bug with no API change → `0.1.9` (patch).
- Add a new harness tool → `0.2.0` (minor).
- Remove or rename a public method → `0.2.0` (minor — breaking is allowed
  pre-1.0, but only ever in a minor).
- From `0.1.8`, **never** jump to `0.1.11` for a fix, and **never**
  release `0.3.0` for a one-line bugfix.
- Two fixes in a row → `0.1.9`, then `0.1.10` — one step each time.
- Dependency example: releasing `ask-ruby-harness` runs
  `gemchain update ask-ruby-harness <ver>` from the workspace root; the
  cascade rewrites this gem's gemspec constraint, re-runs this suite, bumps
  ask-ruby-harness-mcp by one patch, commits, publishes, tags, and pushes —
